...
from .map_encoder import MapEncoder, ContextTransformer
...
class STAR(torch.nn.Module):
    ...
    def __init__(self, args, dropout_prob=0):
        ...
        self.context_transformer = ContextTransformer(dim_model=32)
    ...
    def forward(self, inputs, iftest=False):
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            # get map features using map info
            map_features = self.map_encoder()
            # repeat map features for all samples in the batch
            map_features = map_features.unsqueeze(1).repeat(1, temporal_input_embedded.size(1), 1)
            
            # use map features to enrich current trajectory features, using a transformer
            temporal_input_embedded = self.context_transformer(src=map_features, tgt=temporal_input_embedded)[-1]
