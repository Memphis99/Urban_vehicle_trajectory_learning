...
class MapMatching:
    ...
    def make_map(self, index_edges=True, rtree=True):
        map_con = InMemMap("athensmap", use_latlon=self.match_latlon, index_edges=index_edges,
                           dir=Path('data_pneuma'), use_rtree=rtree)  # Deprecated crs style in algorithm
        ...
        if rtree:
            map_con.rtree = None  # reset rtree
            map_con.setup_index()  # regenerate rtree (when this method is first call in constructor, graph is None)
